...
import type { EventContext, PushMessage } from '../types/common';
...
function buildApnsEnv(context: EventContext): Record<string, any> {
  ...
  return {
    ...
    APNS_PROXY_SECRET: env?.APNS_PROXY_SECRET,
  };
}
    ...
    if (typeof val === 'string') {
      switch (lowerKey) {
        ...
        case 'body':
          ...
          break;
        case 'sound':
          // Compatible with old parameters - normalize sound
          msg.sound = normalizeSound(val);
          break;
        default:
...
async function executePush(
  ...
  try {
    ...
    return {
      code: 400,
      error: `failed to get device token: ${getErrorMessage(error)}`,
    };
  }
  ...
  if (deviceToken.length > 128) {
    ...
    try {
      ...
      logWarn(
        'executePush',
        'Failed to delete invalid device token',
        { deviceKey: msg.deviceKey, error },
        context.env
      );
    }
  ...
  if (result.code === 410 || (result.code === 400 && result.error?.includes('BadDeviceToken'))) {
    try {
      ...
      logWarn(
        'executePush',
        'Failed to mark device token as invalid',
        { deviceKey: msg.deviceKey, error },
        context.env
      );
    }
